      ...
      <div class="modal-dialog modal-lg">
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title mt-0" id="myLargeModalLabel">{{ editMode ? "Edit" : "Add" }} Blog</h5>
          ...
          <form @submit.prevent="editMode ? update() : store()" @keydown="form.onKeydown($event)">
            ...
            <div class="modal-footer">
              ...
              <button :disabled="form.busy" type="submit" class="btn btn-primary">{{ editMode ? "Update" : "Create" }} Blog</button>
            </div>
          </form>
        </div>
      </div>
...
export default {
  ...
  mounted() {
    document.title = 'Our Blog List | Baitulaman';
    this.getAllBlog();
  },
  methods: {
    getAllBlog(){
      this.isLoading = true;
      axios.get(baseurl + 'api/blogs?page='+ this.pagination.current_page).then((response)=>{
    ...
    searchData(){
      axios.get(baseurl + "api/search/blogs/" + this.query + "?page=" + this.pagination.current_page).then(response => {
        this.blogs = response.data.data;
        this.pagination = response.data.meta;
      }).catch(e => {
        this.isLoading = false;
    ...
    store(){
      this.form.busy = true;
      this.form.post(baseurl + "api/blogs").then(response => {
        $("#BlogModal").modal("hide");
        this.getAllBlog();
      }).catch(e => {
        this.isLoading = false;
    ...
    update(){
      this.form.busy = true;
      this.form.put(baseurl + "api/blogs/" + this.form.id).then(response => {
        $("#BlogModal").modal("hide");
        this.getAllBlog();
      }).catch(e => {
        this.isLoading = false;
    ...
    destroy(id) {
      Swal.fire({
        ...
        if (result.isConfirmed) {
          axios.delete('api/blogs/' + id).then((response) => {
            this.getAllBlog();
            Swal.fire(
                'Deleted!',

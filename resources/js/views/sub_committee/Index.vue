...
export default {
  ...
  methods: {
    getAllAdvisor(){
      this.isLoading = true;
      axios.get(baseurl + 'api/sub-committee?page='+ this.pagination.current_page).then((response)=>{
    ...
    searchData(){
      axios.get(baseurl + "api/search/sub-committee/" + this.query + "?page=" + this.pagination.current_page).then(response => {
        this.advisors = response.data.data;
        this.pagination = response.data.meta;
      }).catch(e => {
        this.isLoading = false;
    ...
    store(){
      this.form.busy = true;
      this.form.post(baseurl + "api/sub-committee").then(response => {
        $("#AdvisorModal").modal("hide");
        this.getAllAdvisor();
      }).catch(e => {
        this.isLoading = false;
    ...
    update(){
      this.form.busy = true;
      this.form.put(baseurl + "api/sub-committee/" + this.form.id).then(response => {
        $("#AdvisorModal").modal("hide");
        this.getAllAdvisor();
      }).catch(e => {
        this.isLoading = false;
    ...
    destroy(id) {
      Swal.fire({
        ...
        if (result.isConfirmed) {
          axios.delete('api/sub-committee/' + id).then((response) => {
            this.getAllAdvisor();
            Swal.fire(
                'Deleted!',
